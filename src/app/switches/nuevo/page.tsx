  ...
  async function crearSwitchAction(
    ...
    const parseNumberField = (
      ...
      if (Number.isNaN(parsed)) {
        return {
          ok: false,
          message: `Introduce un número válido en "${etiqueta}".`,
        };
      }
    ...
    const parseIntegerField = (
      ...
      if (Number.isNaN(integerValue)) {
        return {
          ok: false,
          message: `Introduce un número entero válido en "${etiqueta}".`,
        };
      }
    ...
    const ubicacionId = parseIntegerField("ubicacion_id", "Ubicación");
    if (!ubicacionId.ok) {
    ...
    if (typeof enGarantiaRaw === "string") {
      const normalized = enGarantiaRaw.trim().toLowerCase();
      if (normalized === "true") enGarantia = true;
      else if (normalized === "false") enGarantia = false;
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-10">
      <SwitchEditForm
        ...
        title="Nuevo switch"
        description="Introduce los detalles del switch y guarda para continuar con la configuración de puertos."
