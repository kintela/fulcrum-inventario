...
function getSupabaseConfig(): SupabaseConfig {
  ...
  const rawServiceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY ?? null;
  const serviceRoleKey =
    typeof rawServiceRoleKey === "string" && rawServiceRoleKey.trim().length > 0
      ? rawServiceRoleKey.trim()
      : null;

  if (!url || !publicAnonKey) {
    throw new Error(
      "Supabase URL o clave anónima no configuradas. Añádelas al fichero .env.local.",
    );
  }

  // En servidor preferimos service_role para que RLS pueda cerrarse al exterior.
  const anonKey =
